...
for course in COURSES:
    ...
    if st.button(
        ...
        key=f"fy_plan_{course['canvas_id']}",
    ):
        ...
        st.session_state[f"fy_plan_{course['canvas_id']}"] = plan

    plan = st.session_state.get(f"fy_plan_{course['canvas_id']}")
    if plan:
        for idx, a in enumerate(plan.get("priority_actions", []), 1):
            with st.container(border=True):
                st.markdown(f"**{idx}. {a['action']}**")
                st.caption(f"⏱ {a.get('duration_min', '?')} min  ·  {a.get('reason', '')}")
                if a.get("expected_outcome"):
                    st.caption(f"🎯 Expected: {a['expected_outcome']}")
        if plan.get("note"):
