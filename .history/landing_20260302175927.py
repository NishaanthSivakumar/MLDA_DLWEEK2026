...
# Header
header_cols = st.columns([2] + [1] * len(weeks))
header_cols[0].markdown("**Course**")
for i, wk in enumerate(weeks):
    header_cols[i + 1].markdown("🌴" if wk == 7 else f"**W{wk}**")
...
for course in COURSES:
    ...
    # Course-side panel (readiness + topic mastery)
    ...
    with left:
        ...
        st.markdown(
            f"""<span class="badge" style="background:rgba(255,255,255,0.08);">{state['state']}</span>
            <div class="small-muted">{state['reason']}</div>""",
            unsafe_allow_html=True,
        )

        st.caption("View your weak/strong topics, weekly plan & professor matches on the **For You** page.")
        if st.button(f"📌 Open For You: {course['name']}", key=f"fy_{course['canvas_id']}"):
            st.session_state["student_id"] = USER_ID
            st.switch_page("pages/for_you.py")

    with right:
        row_cols = st.columns([2] + [1] * len(weeks))
        row_cols[0].markdown("**Status**")
